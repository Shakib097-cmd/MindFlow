      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-indigo-900/80 border border-indigo-700 flex items-center justify-center text-sm font-bold text-indigo-300">
              {user.name?.[0]?.toUpperCase() || user.email[0].toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-white">{user.name || 'MindFlow User'}</h3>
                <span
        ...
        {/* Action Tabs */}
        <div className="px-5 py-2.5 border-b border-slate-800 bg-slate-900/30 flex items-center gap-2">
          <button
            onClick={() => setActiveAction('details')}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
              activeAction === 'details' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-800'
            }`}
          >
            Account Details
          </button>
          {canModify && (
            <>
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                  activeAction === 'suspend' ? 'bg-rose-600 text-white' : 'text-slate-400 hover:text-rose-300 hover:bg-slate-800'
                }`}
              >
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                  activeAction === 'plan' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-blue-300 hover:bg-slate-800'
                }`}
              >
                Change Plan
              </button>
          ...
          {canResetUsage && (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                activeAction === 'resetUsage' ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-amber-300 hover:bg-slate-800'
              }`}
            >
              Reset AI Quota
            </button>
        ...
        <div className="p-6 overflow-y-auto space-y-5 custom-scrollbar flex-1">
          ...
          {activeAction === 'details' && (
            <div className="space-y-4">
              ...
              {isSuspended && (
                <div className="p-3.5 rounded-xl bg-rose-950/50 border border-rose-800/80 text-rose-200 flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-rose-400 shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-xs font-bold text-rose-300">Account is Currently Suspended</h4>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  ...
                  <div className="text-lg font-bold text-white">
                    {user.aiUsage.used} <span className="text-xs font-normal text-slate-400">/ {user.aiUsage.limit}</span>
                  </div>
                ...
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  ...
                  <div className="text-lg font-bold text-white">{user.mapsCount || 0}</div>
                ...
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  ...
                  <div className="text-lg font-bold text-blue-400 uppercase">{user.plan}</div>
                  <p className="text-[10px] text-slate-500 mt-1">Tier Quota</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800 space-y-2.5 text-xs">
                ...
                <div className="flex items-center justify-between text-slate-300">
                  ...
                  <span className="font-mono">{new Date(user.lastActiveAt).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between text-slate-300">
                  <span className="text-slate-500">User Document ID:</span>
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-3">
                ...
                <p className="text-xs text-slate-400">
                  {isSuspended
                    ? 'Reactivating this user will restore their ability to generate AI maps and access protected cloud sync.'
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full h-20 bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-rose-500"
                  />
                </div>
          ...
          {activeAction === 'plan' && (
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Reason for Plan Change</label>
                  <input
                    type="text"
                    value={planReason}
                    onChange={(e) => setPlanReason(e.target.value)}
                    placeholder="e.g. VIP Customer upgrade / Manual grant"
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-xs font-bold text-white"
                  >
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 space-y-3">
                ...
                <p className="text-xs text-slate-400">
                  ...
                  <strong className="text-white">{user.aiUsage.used}</strong> back to <strong className="text-emerald-400">0</strong>.
                </p>
                ...
                <div className="flex items-center justify-end gap-2 pt-2">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-4 py-1.5 rounded-lg bg-amber-600 hover:bg-amber-500 text-xs font-bold text-white"
                  >
